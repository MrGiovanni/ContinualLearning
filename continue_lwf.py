...
def train(args, train_loader, model, optimizer, loss_func_dice, loss_func_bce, loss_func_ce):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        if args.out_nonlinear == 'sigmoid':
            print(torch.nonzero(y[:, :, 48, 48, 48] == 1)[:, 1] + 1)
            ...
            old_dice = loss_func_dice(logit_map, y, args.old_organ_list)
            old_bce = loss_func_bce(logit_map, y, args.old_organ_list)
            loss = term_seg_BCE + term_seg_Dice + old_dice + old_bce
            loss_bce_ave += term_seg_BCE.item() + old_bce.item()
            loss_dice_ave += term_seg_Dice.item() + old_dice.item()
            ...
            # old_label = y.new_zeros((b, d, h, w), dtype=torch.long)
            # for icls in args.old_organ_list:
                # old_label[y[:, icls-1] == 1] = icls
            
            print(label[:, 48, 48, 48])
            term_seg_Dice = loss_func_dice.forward(logit_map[:, 1:], y, args.organ_list)
            term_seg_CE = loss_func_ce(logit_map, label)
            old_dice = loss_func_dice(logit_map[:, 1:], y, args.old_organ_list)
            # old_ce = loss_func_ce(logit_map, old_label)
            loss = term_seg_Dice + term_seg_CE + old_dice
    ...
    if args.out_nonlinear == 'sigmoid':
        ...
        print('Epoch=%d: ave_dice_loss=%2.5f, ave_ce_loss=%2.5f' % (args.epoch, loss_dice_ave/len(epoch_iterator), loss_ce_ave/len(epoch_iterator)))
...
def process(args):
    ...
    weight = torch.ones(args.out_channels).cuda()
    # weight[27] = 100
    loss_func_ce = nn.CrossEntropyLoss(weight)
    ...
    if args.model in ['swinunetr_partial', 'our_onehot'] :
        ...
        optimizer = torch.optim.AdamW(
            [{'params': class_params, 'lr': args.lr},
            {'params': model_params}],
...
def main():
    ...
    parser.add_argument('--old_organ_list', nargs='+', type=int, required=True)
